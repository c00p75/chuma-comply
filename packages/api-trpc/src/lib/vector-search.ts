 ...
 */
export function getVectorSearchConfig(): VectorSearchConfig | null {
...
export async function hybridVectorSearch(
  queryEmbedding: number[],
  allChunks: Array<{ chunkId: string; embedding: number[]; [key: string]: any }>,
  topK: number = 10,
  filter?: Record<string, any>
): Promise<Array<{ chunkId: string; similarity: number; chunk: any }>> {
