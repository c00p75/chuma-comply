...
export function getFinalGenerationPrompt(
  query: string,
  chunks: Array<{ content: string; sourceDocument: string; sectionNumber?: string; actName?: string }>
): string {
...
export function getFinalGenerationPromptJSON(
  query: string,
  chunks: Array<{ content: string; sourceDocument: string; sectionNumber?: string; actName?: string; regulatoryBody?: string }>
): string {
  const context = chunks
    .map((chunk, idx) => {
      const source = chunk.actName || chunk.sourceDocument;
