 ...
 */
function normalizeSourceTitle(llmSourceTitle, validSources) {
 ...
 */
function validateAndTransformChecklistItem(item, index, validSources) {
    try {
 ...
 */
function validateAndTransformResponse(parsedResponse, topChunks) {
    ...
    if (parsedResponse.checklist && Array.isArray(parsedResponse.checklist)) {
        const validatedItems = parsedResponse.checklist
            .map((item, index) => validateAndTransformChecklistItem(item, index, validSources))
    ...
    getComplianceChecklist: protectedProcedure
        ...
        try {
            ...
            if (filteredDocs.empty) {
                ...
                let fallbackQuery = vectorChunksRef;
